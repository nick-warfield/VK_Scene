...
// rectangle with normalized device coordinates
...
public:
    HelloTriangleApplication(std::filesystem::path p)
	: root(p)
    {
    }
    void run()
...
private:
	...
	stbi_uc* pixels = stbi_load(
	    (root / "resources/infest_the_rats_nest.jpg").c_str(),
	    &texWidth,
	    &texHeight,
	    &texChannels,
	    STBI_rgb_alpha);
	...
	descriptorSets.reserve(swapChainImages.size());
	...
	for (size_t i = 0; i < swapChainImages.size(); i++) {
		...
		device,
		static_cast<uint16_t>(descriptorWrites.size()),
	...
	for (size_t i = 0; i < commandBuffers.size(); ++i) {
	    ...
	    // (_, vertex count, instance count, first vertex, first instance)
